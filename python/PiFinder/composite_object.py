...
class CompositeObject:
    ...
    # Planets (PL), named stars (Str), and observing-list coordinates (OBS)
    # show their own name; all other catalogs use the "<code> <sequence>" format.
    @property
    def display_name(self):
        """
        Returns the display name for this object
        """
        if self.catalog_code in ("PL", "Str", "OBS") and self.names:
            return self.names[0]
